...
discarded, and what leaves the operation. For **IO-aware tiled exact attention**, the key question
...
paper-specific state or transformation runs, and an assertion checks **online softmax statistics produce the same result as a numerically stable reference**.
The equation `softmax(QKᵀ)V` is the compact specification for that function. Trace
